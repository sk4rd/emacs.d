...
You need to install the =FantasqueSansMono Nerd Font= for the correct
font display.

Remember to [[*all-the-icons][install the missing icons]] upon the initial start.
...
** helm-mode
Helm mode is an auto completion layer for emacs searches. I find it
very useful.
#+begin_src elisp :tangle init.el
  (use-package helm
    :config (helm-mode))
